...
RSpec.describe OpenHAB::DSL::Rules::Rule do
  describe "#remove_rule" do
    it "works" do
      ...
      expect(described_class.script_rules.map(&:uid)).to eql [my_rule.uid]
      remove_rule(my_rule)
    ...
    it "cleans up timers for a duration condition when the rule" do
      ...
      expect(OpenHAB::DSL::Timers.timer_manager.active_timer_count).to be 0
      Item1.on
      expect(OpenHAB::DSL::Timers.timer_manager.active_timer_count).to be 1
      remove_rule(my_rule)
      expect(OpenHAB::DSL::Timers.timer_manager.active_timer_count).to be 0
    end
  end
